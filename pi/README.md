...
|---------|-------------|----------|----|---------|  
| グローバル指示 | CLAUDE.md | `../AGENTS.md`（root 共通） | `../AGENTS.md` + `AGENTS.pi.md`（連結デプロイ） | ✅ 統一済み |
...
グローバル指示は repo ルートの `../AGENTS.md`（共通部）+ `pi/AGENTS.pi.md`（pi 固有）。
デプロイ時に連結して `~/.pi/agent/AGENTS.md` を生成する:

```bash
{ cat ../AGENTS.md; echo; cat AGENTS.pi.md; } > ~/.pi/agent/AGENTS.md
```

```
pi/
├── AGENTS.pi.md                   # ✅ pi 固有グローバル指示（root AGENTS.md 末尾に連結）
├── skills/
